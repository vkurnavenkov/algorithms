...
class LogParser
  ...
  def unique_by_date_counts_to_csv(csv_filename)
    result = Hash.new { |h, k| h[k] = 0 }
    File.open(@filename) do |f|
      until f.eof?
        line = f.readline
        regex = line.match(/^(?<date>\S{3}\s\d{2}\s\d{2}:\d{2})/)
        result[regex['date']] += 1 if regex
      end
    end
...
log = LogParser.new('messages.log')
log.unique_by_date_counts_to_csv('messages.csv')
