...
public class Codec {
    ...
    private String preorderDfs(TreeNode root, String data) {
        if (root == null) return data + "N,";

        data += Integer.toString(root.val) + ",";
        data = preorderDfs(root.left, data);
        data = preorderDfs(root.right, data);
        return data;
    }
    ...
    private TreeNode doDeserialize(List<String> values) {
        ...
        node.left = doDeserialize(values);
        node.right = doDeserialize(values);
