// Coursera. Advanced Algorithms and Complexity -> Week 2 -> Programming Assignment 2
//
...
class LinearEquationsSolver {
  ...
  double[] b; // output matrix for coefficient
  ...
  private void processSubset(Set<Integer> subset){
    ...
    if (!gElim.hasSolution) return;

    double[] temp_result = gElim.b;
    if (!satisfiesAllInEq(temp_result)) return;
  ...
  void combinationUtil(int[] arr, int n, int r, int index, int[] data, int i)
  {
    ...
    if (index == r)
    {
      ...
      for (int j=0; j<r; j++) set.add(data[j]);
      processSubset(set);
      return;
    }
